...
class HouseMama():
    ...
    def get_items(self, city_code):
        ...
        url = 'https://housemama.com.tw/Function/Ajax/SearchObj.aspx'
        headers = {
        ...
        while page<last_page:
            page+=1
            data = f'&HotObj=0&TabID=Obj_List&RS_Type=2&CityID={city_n}&AreaID_List=&EventType=AllObj&Pg1_ID=Pg1&Pg1={page}&OrderS_ID=OrderS&OrderS=委託起日_A&RawUrl=%2Fsell_item&rnd=0.545184720109688'
            try:
                ...
                for item in items:
                    link = 'https://housemama.com.tw' + item.find('a')['href']
                    subject = item.find('a').text
                    source_id = re.findall('/sell_item/(.+)', link)[0].strip('/')
                    price1 = item.find('dd',{'class':'ShowBuy'}).find('h4').get_text(strip=True)
                    match = re.findall('總價(\d+\.?\d*)萬', price1)
                    if match:
                        re_price = int(match[0])
                    else:
                        re_price = int(item.find('span',{'class':'discount'}).text)
                    obj_data = {
                                "source_id" : source_id,
                                "subject" : subject,
                                "link" : link,
                ...
                if pages:
                    last_page = int(re.findall(f"顯示頁數 {page} - (\d+)", pages.text)[0])
        ...
        for obj in list_obj:
            ...
            try:
                ...
                if is_feature:
                    ...
                    if not feature:
                        feature = ''
                parking_type, manage_fee, blockto, house_num = '', '', '', ''
                ...
                if match:
                    blockto = match[0]
                match = re.findall('每層戶數(\d+) 戶', details[0].text)
                if match:
                    house_num = match[0]
            ...
            # 此網站無提供以下資訊
            manage_type, edge, dark, contact_type, contact_man, community = '', '', '', '', '', ''
