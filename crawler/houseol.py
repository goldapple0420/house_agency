...
class HouseOl():
    ...
    def get_items(self, city_code):
        ...
        url = 'https://www.houseol.com.tw/Function/Ajax/SearchObj.aspx'
        ...
        while page<last_page:
            ...
            data = {
                ...
                'Pg1': str(page),
                'OrderS_ID': 'OrderS',
                'OrderS': '委託起日_A', # 按刊登日期排序
                'RawUrl': '/sell_item',
            }
            s = requests.session()
            time.sleep(random.uniform(3,6))
            try:
                res = s.post(url, data=data, headers=header, proxies=proxy, verify=False, timeout=(5,10))
                res.raise_for_status()
                print(f'開始擷取{self.source} {city} P{page} 物件')
                soup = BeautifulSoup(res.text, 'lxml')
                items = soup.find_all('li',{'class':'house_block'})
                # print(f'City{city} P{page} {len(items)}個物件')
                for item in items:
                    link = 'https://www.houseol.com.tw' + item.find('a')['href']
                    ...
                    if price1 == '總價來電洽詢':
                        ...
                        if match:
                            ...
                            price2 = item.find('span',{'class':'discount'}).text
                            if price2 == '來電洽詢':
                                re_price = -1
                            else:
                                re_price = int(float(price2))
                    obj_data = {
                                "source_id" : source_id,
                                "subject" : subject,
                                "link" : link,
                ...
                self.logger.error(f"HTTP Error processing {city} Page {page}: {str(err)}")
    ...
    def get_data(self, city_code):
        ...
        for obj in list_obj:
            try:
                ...
                res = requests.get(link, headers=header, proxies=proxy, verify=False, timeout=10)
                res.raise_for_status()
                if '下架' in res.text:
                    # print('連結出錯了，或已下架！')
                    continue
