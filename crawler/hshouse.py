...
class HShouse():
    ...
    def get_items(self, city_code):
        ...
        url = 'https://www.hshouse.com.tw/Function/Ajax/SearchObj.aspx'
        ...
        while page<last_page:
            ...
            try:
                ...
                for item in items:
                    link = 'https://www.hshouse.com.tw' + item.find('a')['href']
    ...
    def get_data(self, city_code):
        ...
        for obj in list_obj:
            try:
                ...
                total = float(re.findall(r'(\d+(\.\d+)?)萬',objs.text.split('\n')[2])[0][0])
                ...
                total_ping = float(match[0].lstrip(' (含車位)')) if match else 0
                ...
                match = re.findall('主建物(\d+\.?\d*)\s?坪', details[1].text)
                if match:
                    building_ping = float(match[0])
                match = re.findall('附屬建物(\d+\.?\d*)\s?坪', details[1].text)
                if match:
                    att_ping = float(match[0])
                match = re.findall('公共設施(\d+\.?\d*)\s?坪', details[1].text)
                if match:
                    public_ping = float(match[0])
                match = re.findall('地坪(\d+\.?\d*)\s?坪', details[1].text)
                if match:
